...
// The main function `runChannelInspection` needs to be rewritten.
// Let's replace the whole `runChannelInspection` block.
...
export async function runChannelInspection(channelData: {
  ...
  if (channelData.youtubeUrl || channelData.channelId) {
    if (channelData.youtubeUrl && (channelData.forceLiveFetch || links.length === 0 || bio.length < 20)) {
      try {
        ...
        const liveData = await fetchLiveYouTubeChannelData(channelData.youtubeUrl);
        if (liveData) {
  ...
  if (directBioInvite) {
    ...
    return { foundInvite: directBioInvite, foundLocation: 'CHANNEL_ABOUT', steps, extractedThumbnailUrl };
  } else {
  ...
  if (links.length > 0) {
    ...
    for (const link of links) {
      ...
      if (inv) {
        ...
        return { foundInvite: inv, foundLocation: 'CHANNEL_LINKS', steps, extractedThumbnailUrl };
      }
    }
  ...
  if (videoDescs.length > 0) {
    ...
    for (let i = 0; i < Math.min(5, videoDescs.length); i++) {
      ...
      if (inv) {
        ...
        return { foundInvite: inv, foundLocation: sourceName, steps, extractedThumbnailUrl };
      }
    }
  ...
  if (channelData.pinnedComment) {
    ...
    if (inv) {
      ...
      return { foundInvite: inv, foundLocation: 'PINNED_COMMENT', steps, extractedThumbnailUrl };
    }
  ...
  if (websiteUrls.length > 0) {
    ...
    for (const item of websiteUrls) {
      ...
      const crawlRes = await crawlExternalLinks([item.url], []);
      if (crawlRes.foundInvite) {
        ...
        return { foundInvite: crawlRes.foundInvite, foundLocation: locName, steps, extractedThumbnailUrl };
      }
    }
  ...
  if (socialBioUrls.length > 0) {
    ...
    for (const item of socialBioUrls) {
      ...
      const crawlRes = await crawlSocialBios([item.url], []);
      if (crawlRes.foundInvite) {
  ...
  return { foundInvite: null, steps, extractedThumbnailUrl };
}
`;
