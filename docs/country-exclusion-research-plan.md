...
5. Policy-edit reconciliation: bounded re-evaluation on exclusion removal with a set-aware cohort (rows rejected by `AGGREGATED_CONTENT_LANGUAGE` whose recorded `candidateCountries` contains the removed country); rejection trail records `{source, candidateCountries, representative}` to make the cohort queryable. Forward-only documentation + operator recheck is the fallback if reconciliation is deferred; `removeExcludedCountry` stays a list-only write either way.
...
- Persisted-row reconciliation: channel persisted REJECTED-for-India → India removed → bounded re-evaluation yields non-REJECTED (short-circuit honors the live list); Urdu row recorded as representative India → Pakistan removed → caught via recorded set membership → non-REJECTED; Bengali row recorded as Bangladesh → India removed → non-REJECTED; still-excluded country → stays REJECTED; removal never mass-rewrites projections.
