...
- Website country W present: emit a P3 language item with `detectedCountry` set to the exact website string W when W ∈ set (agreement → unanimity decides); when W ∉ set, emit the item with the set's representative country instead — the disagreement must be EXPLICIT, never silenced, because a lone excluded website would otherwise reject unopposed while strong language evidence contradicts it.
...
- Single-country languages follow the same rule uniformly: website present → emit the website string if it equals the language country, else emit the language country (explicit disagreement); website absent → emit the language country.

**Live-list rule (mutable exclusions).** The approved map defines candidate sets only — it never assumes exclusion. The all-members-excluded check and the final `excluded` lookup evaluate against the LIVE `excluded_countries` list passed into every `assessChannelCountry` evaluation, so operator changes via `removeExcludedCountry` / the Countries API take effect immediately: if India is removed, Hindi loses all rejection power, Urdu/Bengali sets are no longer all-excluded (→ PROCESS, website-agreed W still resolves via the website item alone under existing rules), and single-country languages fall back to the existing lookup. For future evaluations no stale policy persists past the edit; already-persisted `REJECTED` rows are governed by the reconciliation rule in §11, not rewritten by the edit itself.
...
YES → Is there P3 website country evidence?
      YES → website country ∈ language set? YES → REJECT (agree; language item
            carries the website string, unanimity holds) / NO → emit language
            item anyway (explicit disagreement) → unanimity breaks → UNCERTAIN
            or top-confidence country wins, never a language-driven REJECT
...
Unchanged mechanics, extended input: official metadata (P1) and bio (P2) outrank language — when conclusive, the language path is skipped before it can compete. Website evidence shares tier P3 with the new language source, so the existing unanimity rule governs their interaction with no new logic — and disagreement is always explicit, never silenced: the language item is emitted with its own country (or set representative) even when the website disagrees, so a lone excluded website can never reject unopposed against contradicting language evidence. **Multi-country agreement:** the language evidence carries its candidate-country set; agreement with P3 website country W means W ∈ set (Urdu + `.pk`/`.in`, Bengali + `.bd`/`.in`) and is emitted with the exact website string; a website country outside the set still gets an explicit language item carrying the set representative, which breaks unanimity by construction. With no website evidence, the all-members-excluded property (§6) authorizes REJECT with the set named in reasoning — no single country is invented. Outcomes: agreement → gate evaluates unanimously; disagreement → not unanimous → no language-based REJECT (tie → UNCERTAIN; near-miss → existing top-confidence country wins for CONFIRMED/LIKELY, still never a language-driven REJECT). Lower tiers (P4–P9) never block or outvote a decisive P3, exactly as today. `mergeCountryValidationResults` prevents weaker live evidence from overriding; target mismatch can never create REJECTED. Language evidence additionally self-voids on any second eligible language with ≥2 votes. **Non-override rule:** language-based rejection applies only when P1–P2 evidence is absent or inconclusive and P3 website evidence is absent or agrees per the set rule above. If strong country evidence conflicts with the aggregated language evidence, the result is PROCESS unless the existing country-validation rules independently justify REJECT.

**Policy-edit reconciliation (persisted rows) — correction.** The live-list rule governs FUTURE `assessChannelCountry` evaluations (new candidates, hydration re-runs) — immediate there. It does NOT by itself restore already-persisted `REJECTED` rows, and the existing boundary-recovery replay does NOT either: `classifyReconciliationState` short-circuits any policy rejection (`country_status='REJECTED'` without a pinned target-boundary rejection) to `LEGITIMATE_REJECTION` with confidence 100 *before* consulting the live exclusions list (`server/countryBoundaryRecovery.ts:58-65`), so a channel rejected for India stays rejected after India is removed. An earlier draft of this plan claimed otherwise; that claim was wrong. The implementation phase must therefore include explicit reconciliation with a bounded cohort covering ALL policy rejections invalidated by the removal, not only the new language path: on exclusion removal of country X, re-evaluate (a) persisted rows whose recorded rejection source is `AGGREGATED_CONTENT_LANGUAGE` and whose recorded `candidateCountries` set contains X — NOT only rows whose representative country left the list (an Urdu row recorded as representative India must still be caught when Pakistan is removed, likewise Bengali/Bangladesh rows when India is removed); and (b) persisted rows rejected on explicit single-country evidence (P1 official metadata, P2 bio, P3 website) whose recorded rejection country is X. Preserved untouched: rejections for still-excluded countries, HUMAN_REJECTED / NON_TRADING / other terminal states, and rows whose evidence never named X. Re-evaluation yielding non-REJECTED restores the row to the normal pipeline; `removeExcludedCountry` itself must not mass-rewrite channel projections without that bounded re-evaluation. Implementation phase will add coverage: persisted REJECTED-for-India row → India removed → re-evaluation yields non-REJECTED (short-circuit respects the live list); Urdu row (representative India) → Pakistan removed → re-evaluated via set membership → non-REJECTED; Bengali row (representative Bangladesh) → India removed → non-REJECTED; explicit-bio Pakistan row → Pakistan removed → non-REJECTED; HUMAN_REJECTED and still-excluded rows → untouched; country re-added → rule applies again to future evaluations only.
...
5. Policy-edit reconciliation: bounded re-evaluation on exclusion removal with a cohort covering all invalidated policy rejections — language rows via recorded `candidateCountries` set membership plus explicit single-country (P1–P3) rows via recorded rejection country; HUMAN_REJECTED, still-excluded, and unrelated rows untouched. Rejection trail records `{source, candidateCountries, representative}` (language path) to make the cohort queryable. Forward-only documentation + operator recheck is the fallback if reconciliation is deferred; `removeExcludedCountry` stays a list-only write either way.
6. Estimated surface: ~120 lines in `countryInference.ts`, ~15 in `countryValidator.ts`, keyword-list data additions, reconciliation step, tests below.
...
- Persisted-row reconciliation: channel persisted REJECTED-for-India → India removed → bounded re-evaluation yields non-REJECTED (short-circuit honors the live list); Urdu row recorded as representative India → Pakistan removed → caught via recorded set membership → non-REJECTED; Bengali row recorded as Bangladesh → India removed → non-REJECTED; explicit-bio Pakistan row → Pakistan removed → non-REJECTED; HUMAN_REJECTED and still-excluded rows → untouched; removal never mass-rewrites projections.
...
- Policy removal is forward-only by default: already-persisted REJECTED rows are unaffected until the bounded reconciliation above runs; operators must know removal does not itself restore hidden channels.
