...
**Policy-edit reconciliation (persisted rows).** The live-list rule governs FUTURE `assessChannelCountry` evaluations (new candidates, hydration re-runs, recovery replays) — immediate there. It does NOT rewrite history: channels already persisted with `country_status='REJECTED'` stay hidden via the serving filter until re-evaluated, because terminal states are never rescanned by the normal pipeline. After an operator removes a country, affected rows recover only through the existing boundary-recovery replay (which revalidates against the live list) or explicit operator recheck — `removeExcludedCountry` itself must not mass-rewrite channel projections. Implementation phase will add coverage: persisted REJECTED channel → country removed → re-evaluation yields non-REJECTED; country re-added → rule applies again to future evaluations only.
