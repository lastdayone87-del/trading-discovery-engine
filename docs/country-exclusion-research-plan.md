...
5. Policy-edit reconciliation: bounded re-evaluation of persisted rows whose recorded country left the exclusion list (or explicit forward-only documentation + operator-recheck path) — `removeExcludedCountry` stays a list-only write.
...
- Persisted-row reconciliation: channel persisted REJECTED-for-India → India removed → bounded re-evaluation yields non-REJECTED (short-circuit honors the live list); still-excluded country → stays REJECTED; removal never mass-rewrites projections.
