...
import { getDiscoveryTrustDiagnostics, materializeEvaluationWindow } from './server/discoveryTrustEvaluation';
...
async function startServer() {
  ...
  app.get('/api/discovery/evaluation',async(req,res)=>{try{res.json(await getDiscoveryTrustDiagnostics({windowStart:String(req.query.window_start),windowEnd:String(req.query.window_end)}));}catch(err:any){sendOperationError(res,err);}});
  app.post('/api/discovery/evaluation/snapshots',async(req,res)=>{try{res.status(201).json(await materializeEvaluationWindow({windowStart:String(req.body.windowStart),windowEnd:String(req.body.windowEnd),dimension:req.body.dimension,value:req.body.value,revision:req.body.revision}));}catch(err:any){sendOperationError(res,err);}});
  ...
  app.post('/api/persistent-research/hypotheses',async(req,res)=>{try{res.status(201).json(await proposeResearchHypothesis(req.body));}catch(err:any){res.status(400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/providers',async(req,res)=>{try{res.status(201).json(await registerDiscoveryProvider({...req.body,actor:req.operator!.actorId}));}catch(err:any){res.status(err.message==='PROVIDER_CONFIGURATION_CONFLICT'?409:400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/actions',async(req,res)=>{try{res.status(201).json(await proposeDiscoveryAction(req.body));}catch(err:any){res.status(400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/allocations',async(req,res)=>{res.status(409).json({error:'Allocations are server-owned. Trigger a governed research cycle instead.',code:'SERVER_OWNED_ALLOCATION_REQUIRED',requestId:req.requestId});});
  app.post('/api/persistent-research/outcomes',async(req,res)=>{try{res.status(201).json(await recordDiscoveryOutcome(req.body));}catch(err:any){res.status(400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/coverage-cells',async(req,res)=>{try{res.status(201).json(await upsertCoverageCell(req.body));}catch(err:any){res.status(400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/control',async(req,res)=>{try{res.json(await configurePersistentResearch({...req.body,actor:req.operator!.actorId}));}catch(err:any){res.status(err.message==='RESEARCH_CONFIGURATION_CONFLICT'?409:400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/cycles',async(req,res)=>{try{res.status(202).json(await runPersistentResearchCycle(`operator:${req.operator!.actorId}`));}catch(err:any){res.status(400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/external-nominations',async(req,res)=>{try{res.status(201).json(await recordExternalNominations(req.body));}catch(err:any){res.status(400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  app.post('/api/persistent-research/policy-evaluations',async(req,res)=>{try{res.status(201).json(await evaluatePersistentResearchPolicy({...req.body,actor:req.operator!.actorId}));}catch(err:any){res.status(400).json({error:err.message,code:err.message,requestId:req.requestId});}});
  ...
  app.get('/api/review-reasons',requireReviewer,(_req,res)=>res.json({version:REVIEW_REASON_CATALOG_VERSION,actions:REVIEW_REASON_CATALOG}));
  ...
  const reviewAction=(action:'APPROVE'|'REJECT'|'FORCE_RESCAN'):express.RequestHandler=>async(req,res)=>{try{const reviewer=resolveReviewerIdentity(req.header('x-reviewer-id'))||req.operator?.actorId;if(!reviewer)return res.status(400).json({error:'Reviewer identity is required.',code:'REVIEW_VALIDATION_ERROR',requestId:req.requestId});const result=await decideReview({channelId:req.params.channelId,action,expectedVersion:Number(req.body.reviewVersion),reviewer,reason:String(req.body.reason||''),reviewReasonCode:req.body.reviewReasonCode,reviewReasonVersion:req.body.reviewReasonVersion,reviewReasonOther:req.body.reviewReasonOther,notes:req.body.notes,idempotencyKey:req.header('idempotency-key')||'',creatorType:req.body.creatorType,reasonCodes:Array.isArray(req.body.reasonCodes)?req.body.reasonCodes.map(String):undefined});console.info('[HumanReview] Decision committed',{requestId:req.requestId,channelId:req.params.channelId,action,state:result.review?.state,idempotent:result.idempotent});res.json(result);}catch(err:any){const status=err instanceof ReviewConflictError?409:err instanceof ReviewNotFoundError?404:err?.message?.includes('required')||(err?.message?.startsWith('reviewVersion')||err?.message?.startsWith('reviewReason'))?422:500;console.error('[HumanReview] Decision failed',{requestId:req.requestId,channelId:req.params.channelId,action,status,error:err instanceof Error?err.message:'Unknown error'});res.status(status).json({error:status===500?'The review decision could not be saved.':err.message,code:status===409?'REVIEW_CONFLICT':status===404?'REVIEW_NOT_FOUND':status===422?'REVIEW_VALIDATION_ERROR':'REVIEW_PERSISTENCE_ERROR',requestId:req.requestId});}};
  ...
  app.post('/api/query-intelligence/run-cycle', async (req, res) => {
    try {
      const { country, providerKey, capability, maxRuns } = req.body || {};
      const providerTargetRequested = providerKey !== undefined || capability !== undefined || maxRuns !== undefined;
      if (providerTargetRequested) {
        ...
        if (providerKey !== 'brave-search' || capability !== 'SEARCH_BRAVE_DIRECT' || Number(maxRuns) !== 1) {
      ...
      const result = await runAutonomousDiscoveryCycle(country, providerTargetRequested ? { targetProviderKey: providerKey, requiredCapability: capability, allocationType: 'FRONTIER_CANARY', maxRuns: 1, allowShadowProvider: true } : undefined, req.requestId);
      res.json(result);
    } catch (err: any) {
      sendOperationError(res, err);
    }
