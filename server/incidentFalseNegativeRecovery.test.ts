...
const migration = readFileSync(
  new URL('./db/migrations/091_recover_incident_false_negatives.sql', import.meta.url),
  'utf8'
);

test('false-negative recovery is incident-scoped, bounded and auditable', () => {
  ...
  assert.match(migration, /'CLASSIFICATION_FALSE_NEGATIVE_RESCAN'/);
  assert.match(migration, /'classification_false_negative_v1'/);
