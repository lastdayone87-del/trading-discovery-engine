...
export type CohortDimension='overall'|'provider'|'retrievalSurface'|'allocationOrigin'|'proposalFamily'|'country'|'language'|'sourceFamily'|'concept';
...
export async function captureCompletedRunObservation(queryRunId:string):Promise<boolean>{
  ...
  SELECT l.id,COALESCE((SELECT MAX(o.observation_revision)+1 FROM discovery_evaluation_run_observations o WHERE o.query_run_id=l.id),1),l.decision_id,COALESCE(l.allocation_origin,'LEGACY'),l.proposal_family,
    ...
    l.provider_key,COALESCE(l.retrieval_treatment_origin,'CONTROL'),jsonb_build_object('provider',l.provider_allocation_snapshot,'proposalEvidence',COALESCE(l.proposal_evidence_snapshot,'{}'::jsonb)),COALESCE(l.allocation_at,l.scheduled_at),l.completed_at,now(),l.quota_reserved,l.quota_used,GREATEST(l.sighting_pages,CEIL(l.quota_used/100.0)::int),
...
export async function materializeEvaluationWindow(input:{windowStart:string;windowEnd:string;dimension:CohortDimension;value?:string;revision?:number}) {
  ...
  const bounded=await db.query(`SELECT l.query_run_id::text "queryRunId",l.observation_revision "observationRevision",l.outcome_checksum "outcomeChecksum",l.allocation_origin "allocationOrigin",l.proposal_family "proposalFamily",l.provider,l.allocation_snapshot->'provider'->>'retrievalSurface' "retrievalSurface",l.country,l.language,l.source_families "sourceFamilies",l.canonical_concept "canonicalConcept",l.quota_consumed "quotaConsumed",l.provider_requests "providerRequests",l.execution_ms "executionMs",l.raw_results "rawResults",l.distinct_creators "distinctCreators",l.known_creators "knownCreators",l.new_creators "newCreators",l.relevant_new_creators "relevantNewCreators",l.quality_new_creators "qualityNewCreators",l.confirmed_new_creators "confirmedNewCreators",l.wrong_country_results "wrongCountryResults",l.irrelevant_results "irrelevantResults",l.provider_failed "providerFailed",l.invalid_query "invalidQuery",
    NOT EXISTS(SELECT 1 FROM discovery_evaluation_run_observations p WHERE p.country=l.country AND p.classification_observed_at <= $3 AND (p.completed_at,p.query_run_id)<(l.completed_at,l.query_run_id)) "countryCoverageExpanded",
    l.language IS NOT NULL AND NOT EXISTS(SELECT 1 FROM discovery_evaluation_run_observations p WHERE p.language=l.language AND p.classification_observed_at <= $3 AND (p.completed_at,p.query_run_id)<(l.completed_at,l.query_run_id)) "languageCoverageExpanded",
    l.canonical_concept IS NOT NULL AND NOT EXISTS(SELECT 1 FROM discovery_evaluation_run_observations p WHERE p.canonical_concept=l.canonical_concept AND p.classification_observed_at <= $3 AND (p.completed_at,p.query_run_id)<(l.completed_at,l.query_run_id)) "conceptCoverageExpanded",
    (SELECT COUNT(*)::int FROM jsonb_array_elements_text(l.source_families) sf(value) WHERE NOT EXISTS(SELECT 1 FROM discovery_evaluation_run_observations p WHERE p.source_families ? sf.value AND p.classification_observed_at <= $3 AND (p.completed_at,p.query_run_id)<(l.completed_at,l.query_run_id))) "sourceCoverageExpansionCount"
