...
-- 2. Shadow retrieval policy recommendations (Zero serving authority)
CREATE TABLE IF NOT EXISTS retrieval_policy_shadow_recommendations (
  ...
  actual_config_key TEXT NOT NULL REFERENCES retrieval_configurations(config_key),
  ...
  differs_from_actual BOOLEAN NOT NULL DEFAULT FALSE,
...
-- 3. Derived, idempotent aggregate retrieval-policy neighborhood evidence table
...
-- 4. Extend query_runs and autonomous_query_page_observations with config identity and treatment origin
