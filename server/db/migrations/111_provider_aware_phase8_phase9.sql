...
ALTER TABLE frontier_allocation_decisions
  ...
  ALTER COLUMN provider_reservation_id SET DEFAULT 'legacy-compatible',ALTER COLUMN provider_reserved_amount SET DEFAULT 0,
...
ALTER TABLE query_runs ADD COLUMN IF NOT EXISTS provider_key TEXT NOT NULL DEFAULT 'youtube-search' REFERENCES discovery_provider_registry(provider_key) ON DELETE RESTRICT,
 ADD COLUMN IF NOT EXISTS retrieval_surface TEXT NOT NULL DEFAULT 'YOUTUBE_NATIVE', ADD COLUMN IF NOT EXISTS provider_capability TEXT NOT NULL DEFAULT 'SEARCH_YOUTUBE',
 ADD COLUMN IF NOT EXISTS cost_domain TEXT NOT NULL DEFAULT 'YOUTUBE_DATA_API',
 ADD COLUMN IF NOT EXISTS provider_allocation_snapshot JSONB NOT NULL DEFAULT '{"providerKey":"youtube-search","retrievalSurface":"YOUTUBE_NATIVE","capability":"SEARCH_YOUTUBE","costDomain":"YOUTUBE_DATA_API","continuationOwner":"PHASE_9","historicBackfill":true}'::jsonb,
