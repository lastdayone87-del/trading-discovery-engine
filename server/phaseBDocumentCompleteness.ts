...
export const PHASE_B_DOCUMENT_COMPLETENESS_VERSION = 'phase-b-document-completeness-v1';
...
export function buildPhaseBDocumentCompletenessReport(input: { windowStart: string; cutoffAt: string; metrics: PhaseBDocumentCompletenessMetrics }): PhaseBDocumentCompletenessReport {
  ...
  return { version: PHASE_B_DOCUMENT_COMPLETENESS_VERSION, windowStart: input.windowStart, cutoffAt: input.cutoffAt, ready: reasonCodes.length === 0, servingAuthority: false, assertionAuthority: false, reasonCodes, metrics: input.metrics };
}

export async function inspectPhaseBDocumentCompleteness(input: { windowStart: string; cutoffAt: string }): Promise<PhaseBDocumentCompletenessReport> {
  const start = new Date(input.windowStart), cutoff = new Date(input.cutoffAt);
  if (!Number.isFinite(start.getTime()) || !Number.isFinite(cutoff.getTime()) || start >= cutoff) throw new Error('INVALID_DOCUMENT_COMPLETENESS_WINDOW');
    ...
    const result = await db.query(
      `WITH diagnostics AS (
       ...
       SELECT count(*)::int diagnostics,
         ...
         FROM diagnostics d LEFT JOIN projection p ON p.classification_diagnostic_id=d.id`,
      [input.windowStart, input.cutoffAt]
    );
    const row = result.rows[0] || {};
    const report = buildPhaseBDocumentCompletenessReport({ windowStart: start.toISOString(), cutoffAt: cutoff.toISOString(), metrics: { diagnostics: Number(row.diagnostics || 0), completeProjections: Number(row.complete_projections || 0), missingProjections: Number(row.missing_projections || 0), nonEquivalentProjections: Number(row.non_equivalent_projections || 0), incompleteCoverage: Number(row.incomplete_coverage || 0), missingDiagnosticLineage: Number(row.missing_diagnostic_lineage || 0), pendingDiagnosticObservations: Number(row.pending_diagnostic_observations || 0) } });
