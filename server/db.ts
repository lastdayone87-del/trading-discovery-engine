...
export async function scheduleAutonomousQueryRuns(
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      const eligibleProvider=await client.query(`SELECT 1 FROM discovery_provider_registry WHERE provider_key=$1 AND mode='ACTIVE' AND quota_domain=$2 AND capabilities ? $3 FOR SHARE`,[allocatedProvider.providerKey,allocatedProvider.costDomain,allocatedProvider.capability]);
...
export async function completeQueryRun(runId: string, metrics: {
  ...
  try {
    ...
    if(run.rowCount)await client.query(`UPDATE frontier_allocation_decisions
      SET quota_consumed=$2::integer,provider_consumed_amount=$2::bigint
