...
export async function claimNextJob(workerId:string,types?:string[]):Promise<DurableJob|null>{const db=await getDb(); const client=await db.connect(); try{await client.query('BEGIN'); const res=await client.query(`SELECT * FROM jobs WHERE status='PENDING' AND run_after<=now() AND ($1::text[] IS NULL OR type=ANY($1)) ORDER BY priority DESC,created_at ASC FOR UPDATE SKIP LOCKED LIMIT 1`,[types||null]); if(!res.rowCount){await client.query('COMMIT'); return null;} const job=res.rows[0];const payload=parseJson<Record<string,unknown>>(job.payload,{});const traceId=String(payload.traceId||'');const traceAvailable=traceId?!!(await client.query(`SELECT to_regclass('public.discovery_execution_trace') AS relation`)).rows[0]?.relation:false;if(traceAvailable)await client.query(`INSERT INTO discovery_execution_trace(trace_id,stage,outcome,detail) VALUES($1,'WORKER_POLLING','REACHED',$2)`,[traceId,JSON.stringify({workerId,claimableTypes:types||null})]); const upd=await client.query(`UPDATE jobs SET status='PROCESSING',locked_by=$1,locked_at=now(),attempts=attempts+1,updated_at=now() WHERE id=$2 RETURNING *`,[workerId,job.id]); if(traceAvailable)await client.query(`INSERT INTO discovery_execution_trace(trace_id,stage,outcome,detail) VALUES($1,'QUEUE_CLAIM','REACHED',$2)`,[traceId,JSON.stringify({workerId,jobId:job.id,type:job.type})]);await client.query(`INSERT INTO job_attempts(job_id,attempt_number,status) VALUES($1,$2,'PROCESSING')`,[job.id,upd.rows[0].attempts]); await client.query('COMMIT'); return rowToJob(upd.rows[0]);}catch(e){await client.query('ROLLBACK'); throw e;}finally{client.release();}}
...
export async function failJob(jobId:string,error:any):Promise<void>{const db=await getDb(); const res=await db.query('SELECT attempts,max_attempts FROM jobs WHERE id=$1',[jobId]); if(!res.rowCount)return; const {attempts,max_attempts}=res.rows[0]; const msg=String(error?.message||error).slice(0,2000); const providerRetryAt=Number(error?.retryAt); if(Number.isFinite(providerRetryAt)&&providerRetryAt>Date.now()){await db.query(`UPDATE jobs SET status='PENDING',attempts=GREATEST(0,attempts-1),last_error=$2,locked_by=NULL,locked_at=NULL,run_after=$3,updated_at=now() WHERE id=$1`,[jobId,msg,new Date(providerRetryAt).toISOString()]);}else if(attempts>=max_attempts){await db.query(`UPDATE jobs SET status='FAILED',last_error=$2,locked_by=NULL,locked_at=NULL,updated_at=now() WHERE id=$1`,[jobId,msg]);}else{const seconds=Math.min(900,30*Math.pow(2,Math.max(0,attempts-1))); await db.query(`UPDATE jobs SET status='PENDING',last_error=$2,locked_by=NULL,locked_at=NULL,run_after=now()+($3||' seconds')::interval,updated_at=now() WHERE id=$1`,[jobId,msg,String(seconds)]);} await db.query(`UPDATE job_attempts SET status='FAILED',finished_at=now(),error=$2 WHERE job_id=$1 AND finished_at IS NULL`,[jobId,msg]);}
