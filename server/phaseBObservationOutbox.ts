...
export async function executePhaseBObservation(
  ...
  dependencies: PhaseBObservationProcessorDependencies = {
    recordAssignment: recordRetrievalEvaluationAssignment,
    recordDiagnostic: recordProductionClassification,
    recordGroundTruth: recordEvaluationGroundTruth
  }
): Promise<string> {
...
export async function reconcileMissingGroundTruthObservations(limit = 25): Promise<{ discovered: number; completed: number; failed: number }> {
  ...
  for (const row of missing.rows) {
    try {
      await observeGroundTruthLabelReliably({
        type: 'GROUND_TRUTH_LABEL',
        input: {
          channelId: String(row.channel_id),
          reviewDecisionId: String(row.id),
          label: row.decision === 'APPROVE' ? 'TRADING_CONFIRMED' : 'NON_TRADING',
...
export function triggerPhaseBObservationReconciliation(limit = 25): boolean {
  ...
  void reconcileMissingGroundTruthObservations(limit)
    .then(() => reconcilePendingPhaseBObservations(limit))
...
export function buildPhaseBObservationCompleteness(
  rows: Array<{ observation_type: PhaseBObservationType; captured: number | string; completed: number | string; pending: number | string; missing_result_references?: number | string; oldest_pending_at?: string | Date | null }>,
  groundTruthReviews: { eligible: number; labeled: number; unreconciled: number } = { eligible: 0, labeled: 0, unreconciled: 0 }
): PhaseBObservationCompletenessReport {
  const totals = Object.fromEntries((['RETRIEVAL_ASSIGNMENT', 'PRODUCTION_DIAGNOSTIC', 'GROUND_TRUTH_LABEL'] as PhaseBObservationType[]).map(type => {
    ...
    return [type, {
      ...
      missingResultReferences: Number(row?.missing_result_references || 0),
      ...(row?.oldest_pending_at ? { oldestPendingAt: new Date(row.oldest_pending_at).toISOString() } : {})
    }];
  })) as PhaseBObservationCompletenessReport['totals'];
  return {
    version: PHASE_B_OBSERVATION_OUTBOX_VERSION,
    servingAuthority: false,
    totals,
    groundTruthReviews,
    complete: groundTruthReviews.unreconciled === 0 && Object.values(totals).every(item => item.pending === 0 && item.missingResultReferences === 0 && item.captured === item.completed)
  };
}
...
export async function inspectPhaseBObservationCompleteness(): Promise<PhaseBObservationCompletenessReport> {
  ...
  try {
    ...
    const reviewCoverage = await db.query(
      `SELECT count(*)::int eligible,count(l.id)::int labeled,count(*) FILTER(WHERE l.id IS NULL)::int unreconciled
       FROM channel_review_decisions d
       LEFT JOIN evaluation_ground_truth_labels l ON l.review_decision_id=d.id
       WHERE d.decision IN('APPROVE','REJECT')`
    );
    const report = buildPhaseBObservationCompleteness(result.rows, {
      eligible: Number(reviewCoverage.rows[0]?.eligible || 0),
      labeled: Number(reviewCoverage.rows[0]?.labeled || 0),
      unreconciled: Number(reviewCoverage.rows[0]?.unreconciled || 0)
    });
