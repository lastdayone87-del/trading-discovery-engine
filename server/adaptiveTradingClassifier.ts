...
import { ConfigurableWeightedStrategy, evaluateClassificationStages, type EvidenceItem, type RawChannelInput, type VerificationDecision } from './evidenceEngine';
...
export function classifyAdaptiveShadow(input:RawChannelInput,production:VerificationDecision,terms:AdaptiveTerm[],graphConceptIds:Set<string>=new Set()):ShadowResult{
  ...
  const strategy=new ConfigurableWeightedStrategy(),combined=[...production.positiveEvidence,...production.negativeEvidence,...evidence],collection={...production.evidenceCollection,providers:[...production.evidenceCollection.providers,...[...new Set(evidence.map(item=>item.source))].map(provider=>({provider,availability:'AVAILABLE' as const,evidenceCount:evidence.filter(item=>item.source===provider).length,outcome:'EXECUTED_WITH_EVIDENCE' as const,reasonCodes:['GOVERNED_SHADOW_EVIDENCE']}))]},stages=evaluateClassificationStages(input,combined,collection);const candidate=strategy.evaluateDecision(combined,{globalInstruments:[],globalPlatformsPropFirms:[],globalAdvancedConcepts:[],globalNegativeTerms:[]},input.country||'UNKNOWN',collection,stages);
