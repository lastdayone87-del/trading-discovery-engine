...
test('website crawler remains bounded to eight prioritized follow-up pages', async () => {
  ...
  // Recall-safe budget semantics (PR #434 items 1-2, static bound raised 8→12
  // for same-host recall): the crawl stays bounded (root + 12 follow-ups) but
  // the exhausted budget is recorded as PARTIALLY_INSPECTED with
  // budgetExhausted telemetry — never misrepresented as a fully inspected
  // no-match.
  assert.equal(result.outcome, 'PARTIALLY_INSPECTED');
  assert.equal(calls.length, 13, 'one root page plus at most twelve prioritized follow-up pages may be fetched');
  assert.ok(
