...
export async function runChannelInspection(channelData: {
  ...
  if (websiteUrls.length > 0) {
    ...
    for (const item of websiteUrls) {
      ...
      if (crawlRes.foundInvite) {
        ...
        // Structured crawl result is authoritative — do not re-parse log prose for retention.
        const structured=candidateFromNativeInvite({
          nativeInviteCode:crawlRes.foundInvite,
        ...
        if(alternative&&structured){
          const {candidateId:_,...base}=alternative;
          retained=[makeDiscordCandidate({...base,nativeInviteCode:structured.nativeInviteCode,normalizedLocator:structured.normalizedLocator,extractionConfidence:'RESOLVED'})];
        }
  ...
  if (socialBioUrls.length > 0) {
    ...
    const socialFound=discoveredCandidates.some(c=>c.sourceSurface==='SOCIAL_PROFILES')||acquisitionOutcomes.some(item=>item.surface==='SOCIAL_PROFILES'&&item.outcome==='FOUND');
    if(!socialFound){
