...
test('first runtime 429 quarantines only the failing provider so one independent provider can be tried', () => {
  let now = 1_000;
...
test('second distinct runtime 429 inside confirmation window promotes one shared short pause', () => {
  let now = 1_000;
  const providers = new YouTubeProviderCooldown({initialRateLimitCooldownMs:100,maxRateLimitCooldownMs:400,runtimeRateLimitPauseMs:50,now:()=>now});
...
test('same provider repeating after its local confirmation window does not fabricate cross-provider corroboration', () => {
  let now = 1_000;
  const providers = new YouTubeProviderCooldown({initialRateLimitCooldownMs:100,maxRateLimitCooldownMs:400,runtimeRateLimitPauseMs:50,now:()=>now});
  providers.failed('project-a', 'RATE_LIMITED');
  now = 1_100;
...
test('shared runtime pause does not erase a longer provider-specific daily exhaustion boundary', () => {
  let now = Date.parse('2026-07-30T12:00:00Z');
  const providers = new YouTubeProviderCooldown({initialRateLimitCooldownMs:100,maxRateLimitCooldownMs:400,runtimeRateLimitPauseMs:50,now:()=>now});
  const reset = providers.failed('project-a', 'DAILY_QUOTA_EXHAUSTED');
