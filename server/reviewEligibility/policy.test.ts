...
test('review eligibility admits only evidence-complete unresolved human judgment',()=>{const result=evaluateReviewEligibilityV2(base);assert.equal(result.status,'ELIGIBLE');assert.equal(result.reasonFamily,'HUMAN_AMBIGUITY');assert.equal(result.servingAuthority,true);});
