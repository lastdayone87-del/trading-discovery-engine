...
test('operational enrichment recovery reopens only an old machine-owned failure', () => {
...
test('operational enrichment recovery preserves semantic, human, and completed Discord terminal decisions', () => {
  for (const overrides of [
    ...
    assert.equal(decision.reasonCodes[0], overrides.discord_validation_status === 'COMPLETED'
      ...
      : overrides.trading_status === 'TRADING_CONFIRMED' || overrides.trading_status === 'NON_TRADING' || overrides.trading_status === 'HUMAN_REJECTED' || overrides.country_status === 'REJECTED'
