      ...
      WHERE j.type='RETRY_COMMUNITY_ACQUISITION'
        ...
        AND (
          ...
          OR (
            j.status='COMPLETED'
            AND (
              (c.discord_status='NOT_FOUND' AND c.discord_validation_status='RETRY_PENDING')
              OR (c.discord_status='UNCERTAIN' AND c.scan_status='FAILED' AND c.discord_validation_status='NOT_STARTED')
            )
          )
        )
