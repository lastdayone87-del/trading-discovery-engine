...
export async function setStage2CanaryMode(mode: Stage2CanaryKillSwitchMode, change: Stage2ManualModeChange) {
  ...
  try {
    ...
    await client.query(`INSERT INTO stage2_rate_pressure_canary_events(event_type,mode,actor,reason,payload) VALUES('MODE_CHANGE',$1,$2,$3,$4)`, [mode, change.actor, change.reason, JSON.stringify({ from: row.mode, to: mode, generation: nextGeneration, manualApproval: true })]);
...
export async function abortStage2Canary(reason: Stage2AbortReason, actor = 'system-safety-abort') {
  ...
  try {
    ...
    await client.query(`UPDATE stage2_rate_pressure_canary_subjects SET status='ABORTED',updated_at=now() WHERE status='ACTIVE'`);
    await client.query(`INSERT INTO stage2_rate_pressure_canary_events(event_type,mode,actor,reason,payload) VALUES('ABORT','OFF',$1,$2,$3)`, [actor, reason, JSON.stringify({ previousMode: row.mode, generation: nextGeneration })]);
    await client.query('COMMIT');
...
export async function evaluateStage2CanarySubject(subjectKey: string, evidenceSnapshotChecksum: string) {
  ...
  if (!assignment.assigned) {
    await appendObservation('ALLOCATION_MISS', subjectKey, 'CANARY', { randomizationValue: assignment.randomizationValue });
    return { ...assignment, mode: 'CANARY' as const, reason: 'OUTSIDE_ALLOCATION_BUCKET' as const };
  }

  const reservation = await reserveTreatmentSlot(subjectKey, evidenceSnapshotChecksum, assignment.randomizationValue);
  if (!reservation.reserved) {
    await appendObservation('CAP_REJECT', subjectKey, reservation.mode, { reason: reservation.reason });
    return { ...assignment, assigned: false, mode: reservation.mode, reason: reservation.reason, servingAuthority: false as const };
  }

  await appendObservation('TREATMENT_RESERVED', subjectKey, 'CANARY', { treatmentSlot: reservation.treatmentSlot, randomizationValue: assignment.randomizationValue });
  return { ...assignment, mode: 'CANARY' as const, reason: 'TREATMENT_SLOT_RESERVED' as const, treatmentSlot: reservation.treatmentSlot, servingAuthority: false as const };
}

async function reserveTreatmentSlot(subjectKey: string, evidenceSnapshotChecksum: string, randomizationValue: number): Promise<{reserved:boolean;mode:Stage2CanaryKillSwitchMode;reason:string;treatmentSlot?:number}> {
  const db = await getDb();
  const client = await db.connect();
  try {
    await client.query('BEGIN');
    await client.query('SELECT pg_advisory_xact_lock($1)', [CONTROL_LOCK]);
    const control = await client.query(`SELECT mode FROM stage2_rate_pressure_canary_control WHERE singleton=TRUE FOR UPDATE`);
    const mode = control.rows[0]?.mode as Stage2CanaryKillSwitchMode | undefined;
    if (mode !== 'CANARY') {
      await client.query('ROLLBACK');
      return { reserved: false, mode: 'OFF', reason: 'KILL_SWITCH_OFF_DURING_RESERVATION' };
    }
    const existing = await client.query(`SELECT treatment_slot FROM stage2_rate_pressure_canary_subjects WHERE subject_key=$1`, [subjectKey]);
    if (existing.rows[0]) {
      await client.query('COMMIT');
      return { reserved: true, mode: 'CANARY', reason: 'ALREADY_RESERVED', treatmentSlot: Number(existing.rows[0].treatment_slot) };
    }
    const slot = await client.query(`SELECT s.slot FROM generate_series(1,$1::int) AS s(slot) LEFT JOIN stage2_rate_pressure_canary_subjects t ON t.treatment_slot=s.slot WHERE t.treatment_slot IS NULL ORDER BY s.slot LIMIT 1`, [STAGE2_LIMITED_CANARY_POLICY.maximumTreatmentSubjects]);
    if (!slot.rows[0]) {
      await client.query('ROLLBACK');
    ...
    await client.query(`INSERT INTO stage2_rate_pressure_canary_subjects(subject_key,treatment_slot,allocation_basis_points,randomization_value,evidence_snapshot_checksum) VALUES($1,$2,$3,$4,$5)`, [subjectKey, treatmentSlot, STAGE2_LIMITED_CANARY_POLICY.allocationBasisPoints, randomizationValue, evidenceSnapshotChecksum]);
    await client.query('COMMIT');
...
async function appendObservation(eventType: string, subjectKey: string | null, mode: Stage2CanaryKillSwitchMode, payload: Record<string, unknown>) {
  const db = await getDb();
  await db.query(`INSERT INTO stage2_rate_pressure_canary_events(event_type,subject_key,mode,payload) VALUES($1,$2,$3,$4)`, [eventType, subjectKey, mode, JSON.stringify(payload)]);
}
...
export async function recordStage2CanaryHumanOutcome(input: { subjectKey: string; verdict: 'CONFIRMED_NON_TRADING' | 'GENUINE_TRADING_CREATOR'; actor: string; notes?: string }) {
  if (input.verdict === 'GENUINE_TRADING_CREATOR') {
    await appendObservation('HUMAN_OUTCOME', input.subjectKey, 'CANARY', { verdict: input.verdict, actor: input.actor, notes: input.notes ?? null });
