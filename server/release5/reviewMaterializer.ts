...
export async function materializeEligibleReviewInTransaction(client:QueryClient,input:{channelId:string;eligibilityDecisionId:string}){
  ...
  if(prior&&['APPROVED','REJECTED'].includes(prior.state)){
    const humanDecision=await client.query(`SELECT decided_at FROM channel_review_decisions WHERE channel_id=$1 ORDER BY decided_at DESC LIMIT 1`,[input.channelId]);
    const humanDecidedAt=humanDecision.rows[0]?.decided_at||prior.decided_at||prior.updated_at;
    if(humanDecidedAt&&new Date(eligibility.rows[0].decided_at).getTime()<=new Date(humanDecidedAt).getTime()){
      return {materialized:false,reason:'SUPERSEDED_BY_HUMAN_RESOLUTION',state:prior.state};
    }
  }
