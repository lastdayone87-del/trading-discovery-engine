...
export async function resolveGeminiFreeSemanticCooldownExpiryMs(nowMs:number=Date.now()):Promise<number|undefined>{
  ...
  try{
    const db=await getDb();
    const res=await db.query(`SELECT occurred_at FROM provider_call_events WHERE provider='gemini-free' AND status='RATE_LIMITED' AND COALESCE(request_metadata->>'geminiFreeCooldownDeferral','') <> 'true' ORDER BY occurred_at DESC LIMIT 1`);
