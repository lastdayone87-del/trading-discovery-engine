...
export function rowToChannel(row: any): ChannelRecord {
  const channel: ChannelRecord = {
    ...
    public_about_status: row.public_about_status || 'NOT_ATTEMPTED',
    public_about_checked_at: iso(row.public_about_checked_at),
    public_about_attempts: Number(row.public_about_attempts || 0),
    ...
    community_retry_job_executed_count: row.community_retry_job_executed_count == null ? undefined : Number(row.community_retry_job_executed_count),
    ...
    community_retry_job_retry_reason: row.community_retry_job_retry_reason || undefined,
    community_retry_job_retry_code: row.community_retry_job_retry_code || undefined,
    community_retry_job_reconciliation_status: row.community_retry_job_reconciliation_status || undefined,
...
export async function listChannelsPage(args:ChannelListingFilter&{limit:number;offset:number}):Promise<{items:ChannelRecord[];total:number;revision:string|null}> {
  ...
  const columns=`channel_id,channel_name,youtube_url,country,country_status,confidence_score,discord_status,discord_invite,scan_status,scan_attempts,discovery_source,first_seen,last_checked,subscriber_count,channel_thumbnail_url,quality_score,trading_status,trading_confidence_score,trading_category,country_metadata_status,country_metadata_checked_at,latest_upload_at,uploads_last_30_days,uploads_last_90_days,uploads_last_365_days,activity_band,activity_score,activity_observed_at,discord_discovery_status,discord_candidate_locator,discord_candidate_id,discord_candidate_raw_locator,discord_candidate_type,discord_resolution_status,discord_liveness_status,discord_relevance_status,discord_validation_status,
    ...
    (SELECT payload->>'retryCode' FROM jobs WHERE type='RETRY_COMMUNITY_ACQUISITION' AND payload->>'channelId'=channels.channel_id ORDER BY created_at DESC LIMIT 1) community_retry_job_retry_code,
    (SELECT COALESCE(payload->>'reconciliationStatus','NONE') FROM jobs WHERE type='RETRY_COMMUNITY_ACQUISITION' AND payload->>'channelId'=channels.channel_id ORDER BY created_at DESC LIMIT 1) community_retry_job_reconciliation_status,
    (SELECT payload->>'reconciliationCode' FROM jobs WHERE type='RETRY_COMMUNITY_ACQUISITION' AND payload->>'channelId'=channels.channel_id ORDER BY created_at DESC LIMIT 1) community_retry_job_reconciliation_code,
    (SELECT payload->>'reconciliationReason' FROM jobs WHERE type='RETRY_COMMUNITY_ACQUISITION' AND payload->>'channelId'=channels.channel_id ORDER BY created_at DESC LIMIT 1) community_retry_job_reconciliation_reason`;
...
export async function upsertChannel(channel: ChannelRecord): Promise<void> {
  ...
  ON CONFLICT(channel_id) DO UPDATE SET
    ...
    channel.public_about_status || 'NOT_ATTEMPTED', channel.public_about_checked_at || null, channel.public_about_attempts || 0,
...
export async function claimNextJob(workerId:string,types?:string[],excludedErrorPatterns?:string[],excludedReconciliationStatuses?:string[]):Promise<DurableJob|null>{const db=await getDb(); const client=await db.connect(); let claimed:DurableJob|null=null;try{await client.query('BEGIN'); const res=await client.query(`SELECT * FROM jobs WHERE status='PENDING' AND run_after<=now() AND ($1::text[] IS NULL OR type=ANY($1)) AND ($2::text[] IS NULL OR NOT (type='RETRY_COMMUNITY_ACQUISITION' AND EXISTS (SELECT 1 FROM unnest($2::text[]) pattern WHERE COALESCE(last_error,'') LIKE pattern))) AND ($3::text[] IS NULL OR NOT (type='RETRY_COMMUNITY_ACQUISITION' AND COALESCE(payload->>'reconciliationStatus','NONE')=ANY($3::text[]))) ORDER BY priority DESC,created_at ASC FOR UPDATE SKIP LOCKED LIMIT 1`,[types||null,excludedErrorPatterns||null,excludedReconciliationStatuses||null]); if(!res.rowCount){await client.query('COMMIT'); return null;} const job=res.rows[0];const upd=await client.query(`UPDATE jobs SET status='PROCESSING',locked_by=$1,locked_at=now(),attempts=attempts+1,updated_at=now() WHERE id=$2 RETURNING *`,[workerId,job.id]);await client.query(`INSERT INTO job_attempts(job_id,attempt_number,status) VALUES($1,$2,'PROCESSING')`,[job.id,upd.rows[0].attempts]); await client.query('COMMIT');claimed=rowToJob(upd.rows[0]);}catch(e){await client.query('ROLLBACK'); throw e;}finally{client.release();}
...
/**
 * Per-account Gemini cooldown expiry. Each account's window derives only from
 * RATE_LIMITED rows tagged with its own org label; untagged rows cool no
 * account. A 429 on one account never affects another's window. The
 * pool-wide resolver below is retained for legacy callers.
 */
export async function resolveGeminiOrgSemanticCooldownExpiryMs(orgId:string,nowMs:number=Date.now()):Promise<number|undefined>{
  const cooldownMs=geminiSemanticCooldownMs();
  try{
    const db=await getDb();
    const res=await db.query(`SELECT occurred_at FROM provider_call_events WHERE provider='gemini' AND status='RATE_LIMITED' AND request_metadata->>'geminiOrg'=$1 ORDER BY occurred_at DESC LIMIT 1`,[orgId]);
    if(!res.rows[0]?.occurred_at)return undefined;
    const lastRateLimitMs=new Date(res.rows[0].occurred_at).getTime();
    if(nowMs-lastRateLimitMs>=cooldownMs)return undefined;
    return lastRateLimitMs+cooldownMs;
...
/**
 * Pool-wide Groq semantic cooldown expiry from persisted provider_call_events:
 * one RATE_LIMITED event on any route cools the whole pool. Retained for
 * legacy callers and failures without a known scoped org; per-org callers
 * use resolveGroqOrgCooldownExpiryMs below. Consumed by the ENRICH claim gate
 * and retry timing so deferred jobs wake after the window instead of ticking.
...
/**
 * Quota-organization that produced a provider failure, for per-account retry
 * alignment. Prefers the enrichment wrapper's per-provider entry (survives
 * the evidence-engine boundary), then the raw error sidecar (direct provider
 * paths, attached only on semantic paths). When `providers` is given, wrapper
 * entries must also name one of those providers — so a generic
 * PROVIDER_RATE_LIMIT from an unrelated provider can never resolve to a
 * semantic account. Returns undefined when the failure came from no known
 * scoped account — callers then fall back to the pool-wide window (deferrals)
 * or plain backoff (unattributed rate limits).
 */
export function failedProviderOrg(error:any,reasonCodes:string[],providers?:string[]):string|undefined{
  const entries=Array.isArray(error?.providerFailures)?error.providerFailures:[];
  for(const entry of entries){
    const codes=Array.isArray(entry?.reasonCodes)?entry.reasonCodes.map(String):[];
    if(reasonCodes.some(code=>codes.includes(code))&&(!providers||providers.includes(String(entry?.provider||'')))&&typeof entry?.orgId==='string'&&entry.orgId)return entry.orgId;
  }
  const groqSide=reasonCodes.includes('GROQ_RATE_LIMITED')?(error as any)?.groqOrg:undefined;
  if(typeof groqSide==='string'&&groqSide)return groqSide;
  const geminiSide=(error as any)?.geminiOrg;
  if(typeof geminiSide==='string'&&geminiSide)return geminiSide;
  return undefined;
}
/**
 * Per-organization Groq cooldown expiry. Reads only RATE_LIMITED rows tagged
 * with this org; untagged rows cool no org. Deferral echoes stay excluded. A
 * 429 in one organization never affects another's window.
 */
export async function resolveGroqOrgCooldownExpiryMs(orgId:string,nowMs:number=Date.now()):Promise<number|undefined>{
  const cooldownMs=groqSemanticCooldownMs();
  try{
    const db=await getDb();
    const res=await db.query(`SELECT occurred_at FROM provider_call_events WHERE provider='groq' AND status='RATE_LIMITED' AND COALESCE(request_metadata->>'groqCooldownDeferral','') <> 'true' AND request_metadata->>'groqOrg'=$1 ORDER BY occurred_at DESC LIMIT 1`,[orgId]);
    if(!res.rows[0]?.occurred_at)return undefined;
    const lastRateLimitMs=new Date(res.rows[0].occurred_at).getTime();
    if(nowMs-lastRateLimitMs>=cooldownMs)return undefined;
    return lastRateLimitMs+cooldownMs;
...
export function decideJobFailure(error:any,attempts:number,maxAttempts:number,now=Date.now(),firstFailureAt=now,geminiSemanticCooldownExpiryMs?:number,groqSemanticCooldownExpiryMs?:number,geminiFreeSemanticCooldownExpiryMs?:number):{disposition:JobFailureDisposition;runAfter?:number;operationallyBlocked?:boolean}{
  ...
  if(isRetryableInfrastructureFailure(error)){
    ...
    const isGroqRateLimited=providerReasons.includes('GROQ_RATE_LIMITED');
...
export async function failJob(jobId:string,error:any):Promise<JobFailureDisposition|null>{const db=await getDb(); const res=await db.query('SELECT attempts,max_attempts,created_at FROM jobs WHERE id=$1',[jobId]); if(!res.rowCount)return null; const {attempts,max_attempts,created_at}=res.rows[0]; const msg=String(error?.message||error).slice(0,2000); let geminiSemanticCooldownExpiryMs: number|undefined=undefined; let groqSemanticCooldownExpiryMs: number|undefined=undefined; let geminiFreeSemanticCooldownExpiryMs: number|undefined=undefined; const providerReasons=Array.isArray(error?.providerReasons)?error.providerReasons.map(String):[]; if(providerReasons.includes('SEMANTIC_DEFERRED_RATE_PRESSURE')||providerReasons.includes('GEMINI_CAPACITY_DEFERRED')||(providerReasons.includes('PROVIDER_RATE_LIMIT')&&failedProviderOrg(error,['PROVIDER_RATE_LIMIT'],['gemini_semantic'])!==undefined)){const failedGeminiOrg=failedProviderOrg(error,['SEMANTIC_DEFERRED_RATE_PRESSURE','GEMINI_CAPACITY_DEFERRED','PROVIDER_RATE_LIMIT'],['gemini_semantic']);geminiSemanticCooldownExpiryMs=failedGeminiOrg?await resolveGeminiOrgSemanticCooldownExpiryMs(failedGeminiOrg,Date.now()):await resolveGeminiSemanticCooldownExpiryMs(Date.now());} if(providerReasons.includes('GROQ_RATE_LIMITED')){const failedGroqOrg=failedProviderOrg(error,['GROQ_RATE_LIMITED']);groqSemanticCooldownExpiryMs=failedGroqOrg?await resolveGroqOrgCooldownExpiryMs(failedGroqOrg,Date.now()):await resolveGroqSemanticCooldownExpiryMs(Date.now());} if(providerReasons.includes('GEMINI_FREE_RATE_LIMITED')){geminiFreeSemanticCooldownExpiryMs=await resolveGeminiFreeSemanticCooldownExpiryMs(Date.now());}const decision=decideJobFailure(error,attempts,max_attempts,Date.now(),new Date(created_at).getTime(),geminiSemanticCooldownExpiryMs,groqSemanticCooldownExpiryMs,geminiFreeSemanticCooldownExpiryMs);bumpJobFailureDisposition(decision.disposition);const persistedMessage=decision.operationallyBlocked?`OPERATIONALLY_BLOCKED_RETRY_REQUIRED: ${msg}`:msg;if(decision.disposition==='RETRYING_WITHOUT_ATTEMPT'){await db.query(`UPDATE jobs SET status='PENDING',attempts=GREATEST(0,attempts-1),last_error=$2,locked_by=NULL,locked_at=NULL,run_after=$3,updated_at=now() WHERE id=$1`,[jobId,persistedMessage,new Date(decision.runAfter!).toISOString()]);}else if(decision.disposition==='FAILED'){await db.query(`UPDATE jobs SET status='FAILED',last_error=$2,locked_by=NULL,locked_at=NULL,updated_at=now() WHERE id=$1`,[jobId,persistedMessage]);}else{const seconds=Math.min(900,30*Math.pow(2,Math.max(0,attempts-1))); await db.query(`UPDATE jobs SET status='PENDING',last_error=$2,locked_by=NULL,locked_at=NULL,run_after=now()+($3||' seconds')::interval,updated_at=now() WHERE id=$1`,[jobId,persistedMessage,String(seconds)]);} await db.query(`UPDATE job_attempts SET status='FAILED',finished_at=now(),error=$2 WHERE job_id=$1 AND finished_at IS NULL`,[jobId,persistedMessage]);return decision.disposition;}
