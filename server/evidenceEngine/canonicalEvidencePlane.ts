...
export const CANONICAL_EVIDENCE_SCHEMA_VERSION = 'canonical-evidence-v1';
...
const id=(field:EvidenceFieldType,index:number|undefined,text:string)=>digest(`${CANONICAL_EVIDENCE_SCHEMA_VERSION}|${field}|${index??''}|${text.normalize('NFKC')}`);
...
export function buildCanonicalEvidenceCorpus(input:RawChannelInput):CanonicalEvidenceDocument[]{
  ...
  const add=(field:EvidenceFieldType,text:string|undefined,options:Partial<CanonicalEvidenceDocument>={})=>{
    ...
    out.push({id:id(field,options.index,normalized),field,index:options.index,text:normalized,language,script:options.script,publishedAt:options.publishedAt,contentType:options.contentType,sourceFamilyId:options.sourceFamilyId||channelFamily,sourceEntityId:options.sourceEntityId||input.channel_entity_id});
  };
  add('channel_title',input.channel_name);add('channel_bio',input.description);
  ...
  videos.forEach((video,index)=>{const locator=('id'in video&&video.id)||`${channelLocator}:video:${index}`,sourceFamilyId=('source_family_id'in video&&video.source_family_id)||family('video',locator),sourceEntityId=('source_entity_id'in video&&video.source_entity_id)||input.channel_entity_id,common={index,language:video.language,script:video.script,publishedAt:'published_at'in video?video.published_at:undefined,contentType:'content_type'in video?video.content_type:undefined,sourceFamilyId,sourceEntityId};add('video_title',video.title,common);add('video_description',video.description,common);});
  (input.playlists||[]).forEach((playlist,index)=>{const sourceFamilyId=family('playlist',playlist.id||`${channelLocator}:playlist:${index}`);add('playlist_name',playlist.name,{index,sourceFamilyId});add('playlist_description',playlist.description,{index,sourceFamilyId});});
  (input.transcript_excerpts||[]).forEach((excerpt,index)=>add('transcript_excerpt',excerpt.text,{index,language:excerpt.language,sourceFamilyId:family('video',excerpt.video_id||`${channelLocator}:transcript:${index}`)}));
  ...
  (input.visual_evidence||[]).forEach((visual,index)=>add('visual_evidence',visual.description,{index,sourceFamilyId:family('visual',visual.source_ref)}));
