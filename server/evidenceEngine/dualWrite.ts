...
export async function persistClassificationEvidenceBundle(input: RawChannelInput, decision: VerificationDecision, classificationDiagnosticId?: string) {
  ...
  const creatorFocus = classificationDiagnosticId ? await runCreatorFocusShadow({channelId:input.channel_id,subjectEntityId:coverage.subjectEntityId,diagnosticId:classificationDiagnosticId,productionStatus:decision.status,documents,assertions,coverage}).catch(error=>({enabled:false,servingAuthority:false,error:error instanceof Error?error.message:String(error)})) : {enabled:false,servingAuthority:false};
