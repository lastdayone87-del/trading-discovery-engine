                ...
                filteredChannels.map(c => {
                  ...
                  <tr key={c.channel_id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      ...
                      {!retryReconciliationRequired && (c.scan_status==='FAILED'||c.scan_status==='FAILED_PERMANENT') && c.discord_validation_status==='RETRY_PENDING'&&c.community_retry_job_attempts!=null&&c.community_retry_job_max_attempts!=null&&<div className="text-[10px] text-slate-500">Retry-window attempts: {c.community_retry_job_attempts}/{c.community_retry_job_max_attempts}{c.community_retry_job_run_after&&c.community_retry_job_status==='PENDING'?` · ${automaticRetryDue?'due':'next'} ${new Date(c.community_retry_job_run_after).toLocaleString()}`:''}</div>}
                      {!retryReconciliationRequired && (c.scan_status==='FAILED'||c.scan_status==='FAILED_PERMANENT') && c.discord_validation_status==='RETRY_PENDING'&&c.community_retry_job_execution_count!=null&&<div className="text-[10px] text-slate-500">Observed executions: {c.community_retry_job_execution_count} · capacity deferrals: {c.community_retry_job_deferral_count||0}{c.community_retry_job_last_execution_status?` · last ${c.community_retry_job_last_execution_status.toLowerCase()}`:''}</div>}
