                ...
                filteredChannels.map(c => {
                  ...
                  <tr key={c.channel_id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      ...
                      {!retryReconciliationRequired && (c.scan_status==='FAILED'||c.scan_status==='FAILED_PERMANENT'||c.scan_status==='ENRICHMENT_PENDING') && c.discord_validation_status==='RETRY_PENDING'&&(automaticRetryActive||automaticRetryTerminal)&&c.community_retry_job_execution_count!=null&&<div className="text-[10px] text-slate-500" title="Worker claims = every time a worker picked up this retry (each claim is recorded). Capacity deferrals = claims rejected before any acquisition ran (browser/quota unavailable; they never consume the attempts budget). Executed attempts = claims that actually ran an inspection.">Worker claims: {c.community_retry_job_execution_count} · capacity deferrals: {c.community_retry_job_deferral_count||0} · executed attempts: {Math.max(0,(c.community_retry_job_execution_count||0)-(c.community_retry_job_deferral_count||0))}{c.community_retry_job_last_execution_status?` · last ${c.community_retry_job_last_execution_status.toLowerCase()}`:''}</div>}
