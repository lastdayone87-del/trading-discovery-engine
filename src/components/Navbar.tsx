...
import { DashboardOperationalSummary, QueueStatus, QuotaInfo } from '../types';
...
export const Navbar: React.FC<Props> = ({ activeTab, setActiveTab, queueStatus, quotaInfo, matchingResults, operationalSummary }) => {
  const navItems = [
    ...
    { id: 'results', label: `Channels Table (${matchingResults})`, icon: Table },
        ...
        <div className="py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/80">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {operationalSummary && <span title={`Deployment instance ${operationalSummary.deployment.instance}`} className="px-2 py-0.5 text-[10px] font-mono text-slate-500 dark:text-slate-400 rounded-md border border-slate-200 dark:border-slate-700">
                  {operationalSummary.deployment.environment} / {operationalSummary.deployment.service}
                </span>}
              </div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <div className="px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center gap-2">
              ...
              <span className="font-extrabold font-mono text-slate-900 dark:text-white">{operationalSummary?.storedChannels ?? '—'}</span>
            </div>
            ...
            <div className="px-3 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/50 flex items-center gap-2 text-emerald-800 dark:text-emerald-300">
              ...
              <span className="font-extrabold font-mono">{operationalSummary?.activeDiscords ?? '—'}</span>
            </div>
            ...
            <div className="px-3 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/50 flex items-center gap-2 text-amber-800 dark:text-amber-300">
              ...
              <span className="font-extrabold font-mono">{operationalSummary?.pendingScans ?? '—'}</span>
            </div>
          </div>
