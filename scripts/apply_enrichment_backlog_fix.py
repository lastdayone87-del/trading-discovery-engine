...
p = Path('server/providerResilience.ts')
s = p.read_text()
old = "semanticMaxInlineWaitMs: envMs('GEMINI_SEMANTIC_MAX_INLINE_WAIT_MS', 90000)"
new = "semanticMaxInlineWaitMs: envMs('GEMINI_SEMANTIC_MAX_INLINE_WAIT_MS', 8000)"
if old not in s:
    raise SystemExit('Gemini semantic inline-wait anchor not found')
p.write_text(s.replace(old, new, 1))

p = Path('server/queueManager.ts')
s = p.read_text()
old = """      const enrichmentQuotaUnits=enrichmentStage>=2?202:101;
...
"""
new = """      const candidateAlreadyEnriched=Number(candidate.enrichmentStage||0)>=enrichmentStage;
...
"""
if old not in s:
    raise SystemExit('ENRICH_CHANNEL acquisition block anchor not found')
p.write_text(s.replace(old, new, 1))

Path('server/enrichmentBacklogDrain.contract.test.ts').write_text("""import test from 'node:test';
